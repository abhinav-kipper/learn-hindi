...
function SkillCard({ skill }: { skill: SkillBreakdown }) {
  ...
  return (
    <div
      ...
      className="bg-white rounded-2xl p-5 border border-sky-100 shadow-sm cursor-pointer select-none"
    >
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            <p className="text-sm text-slate-600 mt-3 leading-relaxed">{skill.explanation}</p>
            {skill.more_examples.length > 0 && (
              <div className="mt-3 space-y-2">
                {skill.more_examples.map((ex, i) => (
                  <div key={i} className="text-sm pl-3 border-l-2 border-sky-200">
                    <p className="font-medium text-slate-700">{ex.hindi}</p>
