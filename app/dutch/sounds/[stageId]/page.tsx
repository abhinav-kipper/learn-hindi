...
export default function SoundsStagePage({ params }: { params: Promise<{ stageId: string }> }) {
    ...
    return (
      <div style={{ minHeight: '100vh', background: COLORS.lav, padding: 24 }}>
        <div style={{ maxWidth: 480, margin: '0 auto', fontFamily: FONTS.body, color: COLORS.ink }}>
          Stage not found.{' '}
          <button
            onClick={() => router.push('/dutch/sounds')}
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '16px' }}>
        ...
        {complete && (
          <div style={{ marginTop: 22, display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {next ? (
              <button
                onClick={() => { playSound('swipe'); router.push(`/dutch/sounds/${next.id}`) }}
                style={ctaStyle(theme.primary)}
              >
                next: {next.title} →
              </button>
            ) : (
              <button onClick={() => { playSound('tap'); router.push('/dutch/sounds') }} style={ctaStyle(theme.primary)}>
                back to the ladder →
              </button>
            )}
          </div>
