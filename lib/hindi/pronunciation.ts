import courseData from '@/content/hindi/pronunciation-course.json'

export const CARDS_DONE_KEY = 'hindi-pron-cards-done'
export const EARQUIZ_DONE_KEY = 'hindi-pron-earquiz-done'
...
export interface PronCard {
  ...
  grapheme: string
  hint: string
  anchor: { word: string; en: string; dev?: string }
  kind: CardKind
  note?: string
  /** What the "hear the sound" button speaks (romanized). Defaults to `grapheme`. */
  say?: string
  /** Devanagari for the sound/grapheme — fed to TTS for a correct Hindi accent. */
  dev?: string
}
